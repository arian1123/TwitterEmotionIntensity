...
class GloVeModel():
    ...
    def __fit_to_corpus(self, corpus, vocab_size, min_occurrences, left_size, right_size):
        ...
        print ('**** vocab size = ', len(self.__words))
        ...
        with tf.Session(graph=self.__graph) as session:
            ...
            for epoch in range(num_epochs):
                ...
                for batch_index, batch in enumerate(batches):
                    ...
                    _, _loss = session.run([self.__optimizer, self.__total_loss], feed_dict=feed_dict)
                    loss += _loss
...
if __name__ == '__main__':
    model = GloVeModel(embedding_size=50, context_size=10, min_occurrences=5, learning_rate=0.05, batch_size=512)
    corpus = [l.lower().strip().split() for l in open('test.txt').readlines()]
    model.fit_to_corpus(corpus)
    model.train(num_epochs=500)
    #import cPickle
    import pickle
    pickle.dump({'word':model.words, 'vector':model.embeddings}, open('embed.pkl', 'wb'))
    model.generate_tsne(path='./embed.png')